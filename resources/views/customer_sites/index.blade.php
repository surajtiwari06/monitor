...
<div class="mb-3">
    <div class="float-end">
        @can('create', new App\Models\CustomerSite)
            {{ link_to_route('customer_sites.create', __('create New '), [], ['class' => 'btn btn-success']) }}
        @endcan
    </div>
    <h2 class="page-title">{{ __('Monitor Resource list ,') }} <small>{{ __('app.total') }} : {{ $customerSites->total() }} {{ __('resource') }}</small></h2>
</div>
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                ...
                <div class="col-12">
                    {!! Form::text('q', request('q'), ['placeholder' => __('Search Resource')]) !!}
                </div>
                <div class="col-12">
                    {!! Form::select('vendor_id', $availableVendors, request('vendor_id'), ['placeholder' => '--'.__('ALL Groups').'--']) !!}
                </div>
                <div class="col-12">
